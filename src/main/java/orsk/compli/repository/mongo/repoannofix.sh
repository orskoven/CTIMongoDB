...
TARGET_DIR="/Users/simonbeckmann/IdeaProjects/compliMySQL/src/main/java/orsk/compli/repository/mongo"
...
find "$TARGET_DIR" -type f -name "*.java" | while read -r file; do
  ...
  # Prepare the @Repository annotation for mongo
  mongo_annotation="@Repository(\"mongo${class_name}Repository\")"

  # Insert the mongo annotation before the interface declaration
  sed -i.bak "/public interface $class_name/i\\
$mongo_annotation
" "$file"

  echo "Added mongo annotation to $file"
done
